...
/**
 * Minify png and jpg img
 */
module.exports = () => {
    return gulp.task('img:make-symbols-sprite', done => {
        ...
        if (tars.config.svg.active && tars.config.svg.workflow === 'symbols') {
            return gulp.src(`${svgImagesPath}**/*.svg`)
                ...
                .pipe(skipTaskWithEmptyPipe('img:make-symbols-sprite', done))
        ...
        tars.skipTaskLog('img:make-symbols-sprite', 'SVG is not used or you prefer svg-sprite workflow');
        done(null);
    });
};
